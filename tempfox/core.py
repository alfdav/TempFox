...
def create_aws_profile(profile_name, aws_access_key_id, aws_secret_access_key, aws_session_token=None, region=None, output_format="json"):
    ...
    try:
        ...
        credentials_config.set(profile_name, "aws_secret_access_key", aws_secret_access_key)
...
def generate_profile_name(aws_access_key_id, key_type):
    ...
    key_suffix = aws_access_key_id[-8:] if len(aws_access_key_id) >= 8 else aws_access_key_id
...
def prompt_for_profile_creation(aws_access_key_id, aws_secret_access_key, aws_session_token, key_type):
    ...
    if choice == "1":
        ...
        if profile_exists("default"):
            ...
            overwrite = input("Overwrite existing default profile? (y/n): ").lower().strip()
            if overwrite != 'y':
...
def main():
    ...
    try:
        ...
        parser.add_argument('--list-profiles', action='store_true', help='List all AWS profiles and exit')
        parser.add_argument('--cleanup-profiles', action='store_true', help='Clean up old TempFox profiles and exit')
        parser.add_argument('--no-profile', action='store_true', help='Skip profile creation prompts')
        ...
        if args.list_profiles:
            ...
            if profiles:
                ...
                if tempfox_profiles:
                    ...
                    logging.info("💡 Use --cleanup-profiles to remove old TempFox profiles")
            else:
        ...
        if args.cleanup_profiles:
            ...
            confirm = input(f"\nDelete all {len(tempfox_profiles)} TempFox profiles? (y/n): ").lower().strip()
            ...
            if confirm == 'y':
                ...
                logging.info(f"\n🎉 Cleanup complete! Deleted {deleted_count}/{len(tempfox_profiles)} profiles.")
            else:
        ...
        if test_aws_connection(aws_access_key_id, aws_secret_access_key, aws_session_token):
            ...
            if profile_config:
                ...
                if success:
                    logging.info(f"✅ AWS profile '{profile_config['profile_name']}' created successfully!")
                    ...
                    if profile_config['profile_name'] == "default":
                        ...
                        logging.info(f"  • aws --profile {profile_config['profile_name']} sts get-caller-identity")
                        logging.info(f"  • aws --profile {profile_config['profile_name']} s3 ls")
                        logging.info(f"  • cloudfox aws --profile {profile_config['profile_name']} all-checks")
                    
                    profile_created = True
                else:
                    logging.error(f"❌ Failed to create profile '{profile_config['profile_name']}'")
            ...
            if profile_created:
                ...
                logging.info("=" * 60)
                logging.info(f"✅ AWS profile '{profile_config['profile_name']}' is ready for reuse")
